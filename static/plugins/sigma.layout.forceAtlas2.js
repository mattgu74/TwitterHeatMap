      ...
      switch (self.state.step) {
        ...
        case 6:
          ...
          for(var i=0; i<edges.length; i++) {
            var edge = edges[i];
            var c1 = graph.nodes(edge.source).c;
            var c2 = graph.nodes(edge.target).c;
            if(c1 != null && c1 == c2) {
              // OK
            } else if(c1 == null && c1 == c2) {
              con_count += 1;
              connexes[con_count] = [];
              connexes[con_count].push(edge.source);
              connexes[con_count].push(edge.target);
              graph.nodes(edge.source).c = con_count;
              graph.nodes(edge.target).c = con_count;
            } else if(c1 == null && c1 != c2) {
              connexes[c2].push(edge.source);
              s.graph.nodes(edge.source).c = c2;
            } else if(c2 == null && c1 != c2) {
              connexes[c1].push(edge.target);
              graph.nodes(edge.target).c = c1;
            } else {
              connexes[c1] = connexes[c1].concat(connexes[c2]);
              connexes[c2] = [];
            }
          }

          for(var i=0; i<connexes.length; i++) {
            var min_created = null;
            if(!connexes[i]) { continue; }
            for(var j=0; j<connexes[i].length; j++) {
              var node = graph.nodes(connexes[i][j])
              if(min_created == null || min_created > node.created) {
